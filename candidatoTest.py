...
class testBlackList(TestCase):
    ...
    def test_ping(self):
        ...
        self.assertIn("Pong Candidato", msg)

    def test_consulta_candidatos_perfiles(self):
        headers={
            'Content-Type': 'application/json',
            'Authorization': 'Bearer {}'.format(self.tokenfijo)
        }            
        endpoint_perfil='/candidatos/perfiles'
        lstPerfiles=[1,10,100,200,300,700]
        perfiles={
            "lstPerfiles":lstPerfiles
        }  #[1,10,15,20,30,40,50,70,85]
        solicitud_consultar_perfiles=self.client.post(endpoint_perfil, 
                                                data=json.dumps(perfiles), 
                                                headers=headers)
        ...
        lstCandidatos=respuesta_consultar_perfiles['Candidatos']
        for c in lstCandidatos:
            idPerfil=c['id_perfil']
            self.assertIn(idPerfil, lstPerfiles)
            #if idPerfil not in lstPerfiles:
